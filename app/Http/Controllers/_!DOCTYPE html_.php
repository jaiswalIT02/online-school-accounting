...
<body>
    ...
    <script>
        // Sync table heights so footers align at the same row
        function syncTableHeights() {
            const pageSpreads = document.querySelectorAll('.two-page-spread');
            
            pageSpreads.forEach(function(spread) {
                const receiptTable = spread.querySelector('.page-section:first-child table');
                const paymentTable = spread.querySelector('.page-section:last-child table');
                
                if (receiptTable && paymentTable) {
                    // Reset heights first
                    receiptTable.style.height = '';
                    paymentTable.style.height = '';
                    
                    // Get natural heights
                    const receiptHeight = receiptTable.offsetHeight;
                    const paymentHeight = paymentTable.offsetHeight;
                    const maxHeight = Math.max(receiptHeight, paymentHeight);
                    
                    // Set both tables to same height
                    receiptTable.style.height = maxHeight + 'px';
                    paymentTable.style.height = maxHeight + 'px';
                }
            });
        }
        
        // Run on load
        document.addEventListener('DOMContentLoaded', syncTableHeights);
        
        // Run before print
        window.addEventListener('beforeprint', syncTableHeights);
        
        // Run after page load (fallback)
